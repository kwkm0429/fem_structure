    ...
    while (std::getline(ifsi, str)) {
        ...
        if(is_elements){
        	if(loop == 0){
        		...
        		if(phy[phys_id].dim == 1){
        			// boundary
        			int id1 = std::stoi(list[5]) - 1;
        			int id2 = std::stoi(list[6]) - 1;
        			if(phy[phys_id].name == "wall"){
                        dirichlet_vx[id1] = dirichlet_vx[id2] = 0;
                        dirichlet_vy[id1] = dirichlet_vy[id2] = 0;
                        dirichlet_vz[id1] = dirichlet_vz[id2] = 0;
                        for(int i=0;i<3;i++){
                            neumann_vx[id1][i] = neumann_vx[id2][i] = 0;
                            neumann_vy[id1][i] = neumann_vy[id2][i] = 0;
                            neumann_vz[id1][i] = neumann_vz[id2][i] = 0;
                            neumann_pressure[id1][i] = neumann_pressure[id2][i] = 0;
                        }

        				is_dirichlet_vx[id1] = is_dirichlet_vx[id2] = true;
        				is_dirichlet_vy[id1] = is_dirichlet_vy[id2] = true;
        				is_dirichlet_vz[id1] = is_dirichlet_vz[id2] = true;
                    ...
                    dirichlet_vz[id1] = dirichlet_vz[id2] = dirichlet_vz[id3] = dirichlet_vz[id4] = 0;
                    is_dirichlet_vz[id1] = is_dirichlet_vz[id2] = is_dirichlet_vz[id3] = is_dirichlet_vz[id4] = true;
        		}
        	}
            loop++;
        }
...
int main(int argc, char* argv[]){
    if(argc != 6){
        fprintf(stderr,"Usage: ./2d_msh2dat <msh file> <output directory> <node filename> <elem file name> <bc file name>\n");
        exit(1);
    }
